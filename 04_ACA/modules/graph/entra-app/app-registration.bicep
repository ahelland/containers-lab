...
@description('The managed identity to use for FIC')
...
// Assign the Managed Identity the role (Key Vault Certificate User) to read certificates in the Key Vault
resource kvCertificateUserMiRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(vault.id, webIdentity.id, subscriptionResourceId('Microsoft.Authorization/roleDefinitions', 'db79e9a7-68ee-4b58-9aeb-b90e7c24fcba'))
  scope: vault
  properties: {
    principalId: webIdentity.properties.principalId
    principalType: 'ServicePrincipal'
...
// Assign the Managed Identity the role (Key Vault Secrets User) to read secrets from the Key Vault
resource kvSecretsUserRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(vault.id, webIdentity.id, subscriptionResourceId('Microsoft.Authorization/roleDefinitions', '4633458b-17de-408a-b874-0445c86b69e6'))
  scope: vault
  properties: {
    principalId: webIdentity.properties.principalId
    principalType: 'ServicePrincipal'
...
resource createAddCertificate 'Microsoft.Resources/deploymentScripts@2020-10-01' = {
  ...
  dependsOn: [
    kvMiRoleAssignment
    kvCertificateUserMiRoleAssignment
...
output uamiId string          = webIdentity.properties.clientId
